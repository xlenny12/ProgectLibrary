...
class BookService:
    ...
    def create(self, data: BookCreate, actor_id: str) -> BookPublic:
        ...
        self.repo.save(book)
        audit.log(actor_id, "BOOK_CREATED", {"book": book.model_dump(mode="json")})
    ...
    def update(self, book_id: str, data: BookUpdate, actor_id: str) -> BookPublic:
        ...
        if updated.available_qty > updated.total_qty:
            raise ValueError("Available quantity cannot exceed total quantity.")
        self.repo.save(updated)
        audit.log(actor_id, "BOOK_UPDATED", {"book": updated.model_dump(mode="json")})
