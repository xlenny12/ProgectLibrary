...
def test_sms_noop_when_no_credentials():
    svc = NotificationService()
    result = svc.send_overdue_sms(_make_user(), _make_overdue_borrow())
    assert result is False
    assert audit.replay_events()[-1]["action"] == "SMS_SKIPPED"


def test_email_noop_when_no_credentials():
    svc = NotificationService()
    result = svc.send_overdue_email(_make_user(), _make_overdue_borrow())
    assert result is False
    assert audit.replay_events()[-1]["action"] == "EMAIL_SKIPPED"
